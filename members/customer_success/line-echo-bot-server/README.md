...
- 「まとめ」または「要約」を送ると、返信の先頭に **ログ取得状況**（件数 / グループ内訳）を表示します。
- 「グループ名: テスト会」や「『テスト会』グループをまとめて」のように送ると、ログ内の `groupName` / `group` / `groupId` 等に一致する履歴だけを優先して要約します。
- 一致するグループログが 0 件のときは、返信内にその旨を明示します（設定ミスかデータ欠損かの切り分け用）。
- 取得ログに `groupName` / `groupId` が無い場合は `(グループ情報なし)` として扱います。これは GAS 側の保存項目依存です。
