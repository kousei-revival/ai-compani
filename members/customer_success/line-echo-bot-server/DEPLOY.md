...
### STEP2b: グループログ要約の確認（今回の改善）

1. 友だちグループで会話した後、秘書Botへ `まとめ` または `要約` を送る。
2. 返信先頭に **ログ取得状況**（総件数 / グループ内訳）が出ることを確認する。
3. 特定グループを指定したい場合は、次のように送る:
   - `グループ名: テスト会 をまとめて`
   - `「テスト会」グループの会話を要約して`
4. 一致するグループが無い場合は、**一致0件**の注意文が表示されることを確認する。

**重要（GAS側の前提）:**  
この機能は、GAS が返すログに `groupName` / `group` / `groupId` / `source.groupId` などの情報が含まれている前提です。  
これらが無い場合、秘書Bot側では `(グループ情報なし)` としか判定できません。
...
### STEP3b: 本番フロー整理・セルフチェック（15分放置後／Logs を開かず待つ）

「メッセージしたのに反応しない → Logs を開いたら動いた」ように見えることがあります。**ダッシュボードを開いたからプロセスが起きるわけではなく**、**スリープ解除に数十秒〜1分以上かかっている間にタイムミングが重なった**という見方が近いです。切り分けのために、**あえて Logs を開かず**次を一度だけ試します。

1. **15 分以上**送らずに放置する。  
2. スマホから秘書Botに **1通**送る（短いテキストでよい）。  
3. **最低 1〜2 分**、LINE 上で待つ（この間は **Render の Logs を開かない**）。  
4. 期待する体験（無料枠の実用ラインの目安）: **「考え中…」** が届き、そのあと **push で本文** が届く（遅れてよい）。  
5. **2分以上**何も来ない → そのときだけ **Logs** を開き、下の「ヒアリング項目」をメモしながら読む。

**ヒアリング用チェックリスト（メモ用・秘密は値を書かない）**

| # | 確認する項目 |
|---|----------------|
| 1 環境 | Render は **Docker** か **Native Python** か。 **UptimeRobot** 等で `GET /` を定期実行しているか。 |
| 2 再現 | 何分放置したか。送った文面（例だけ）。送った直後〜2分の見え方（「考え中…」なし／既読のみなど）。**Logs をいつ開いたか**（待たずに開いたなら記載）。 |
| 3 Logs | 同じ時間帯に `=== /callback` や `Received` は **ある／ない**。コンテナの起動・再読み込みに見える行はあるか。 |
| 4 LINE | Developers で **Webhook の利用** はオンか。実メッセージ時のみのエラー／再送の気配はあるか。 |
| 5 Env | **`LINE_CHANNEL_ACCESS_TOKEN`**, **`LINE_CHANNEL_SECRET`**, **`ANTHROPIC_API_KEY`** が Render にあるか（名前だけ確認）。**手動で `PORT` を Environment に足していないか**。 |
...
flowchart LR
  phone[phone_send]
  line[LINE_platform]
  renderW[before_Render_app]
  app[FastAPI_callback]
  bg[background_reply]

  phone --> line --> renderW --> app --> bg
```

| Logs の様子 | 詰まり目安（一言） |
|-------------|---------------------|
| **`POST /callback` 相当の行が無い** | **renderW より手前**（スリープ・到達前・URL・チャネル取り違え）。ウェイク・UptimeRobot・Webhook URL を再確認。 |
| **`/callback` はあるが会話に返信が無い** | **bg**（トークン・Claude・`reply`/`push` の例外）。Logs の `ERROR` / `Failed` を追う。 |
| **`/callback` があり `total_ms` が数十 ms 程度** | ハンドラは速い。**待ち時間不足**や LINE 側の一時状態も疑い、もう一度 **2分待ち** を試す。 |

**結果別の「次の一手」（1つだけやる）**

| 状況 | 次の一手 |
|------|-----------|
| 待てば「考え中…」〜返信まで届く | **無料枠として許容**し、安定化したければ UptimeRobot または有料化。 |
| 待っても何も来ず `/callback` ログも無い | **`GET /` でウェイク**してから再送、または UptimeRobot。**Webhook URL** と **このチャネル**を再確認。 |
| `/callback` があるが返信だけ来ない | Render の **`LINE_CHANNEL_ACCESS_TOKEN`** と **`LINE_CHANNEL_SECRET`**、Logs の **Messaging API / Claude** のエラー行を確認。 |
